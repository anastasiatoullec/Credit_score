...
async def get_incomes_by_customer(customer_id): 
    with engine.connect() as connection:
        results = connection.execute("SELECT i.* FROM income AS i INNER JOIN customer AS c ON i.Customer_ID = c.Customer_ID WHERE c.Customer_ID = '"+customer_id+"'")
        if results:
            results = [
            Income(
                income_id=i[0],
...
@api.get('/customer_numbers',tags=['Requests'])
async def customer_numbers():
    ...
    return {"Number of customers" : records }


@api.get('/customers_credit_numbers',tags=['Requests'])
async def customers_credit_numbers():
    """
    Show customers credit numbers
    """

    connection = sqlite3.connect('credit_customer.db')
    cursor = connection.cursor()
    sqlite_select_query = """
    SELECT  Customer_ID, COUNT(*) as nb FROM credit
    GROUP BY Customer_ID ORDER BY nb DESC LIMIT 10"""
    cursor.execute(sqlite_select_query)
    records = cursor.fetchall()

    return {"Customers credit numbers": records }
